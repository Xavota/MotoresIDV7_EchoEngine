...
DX11BlendState::create(BlendDesc desc)
{
  ...
  static_cast<D3D11_BLEND_OP>(desc.renderTarget[0].blendOp);
  blendDesc.RenderTarget[0].BlendOpAlpha =
  static_cast<D3D11_BLEND_OP>(desc.renderTarget[0].blendOpAlpha);
  blendDesc.RenderTarget[0].DestBlend =
  static_cast<D3D11_BLEND>(desc.renderTarget[0].destBlend);
  blendDesc.RenderTarget[0].DestBlendAlpha =
  static_cast<D3D11_BLEND>(desc.renderTarget[0].destBlendAlpha);
  blendDesc.RenderTarget[0].RenderTargetWriteMask =
  desc.renderTarget[0].renderTargetWriteMask;
  blendDesc.RenderTarget[0].SrcBlend =
  static_cast<D3D11_BLEND>(desc.renderTarget[0].srcBlend);
  blendDesc.RenderTarget[0].SrcBlendAlpha =
  static_cast<D3D11_BLEND>(desc.renderTarget[0].srcBlendAlpha);
